...
export default function PlanetModal({
  ...
  // 랜덤 인구 생성 (완료된 일 개수 기반)
  const population = Math.floor(1000 + completedTasks.length * 500);
  ...
  const industry = industries[category.charCodeAt(0) % industries.length];
  ...
  const feature = features[category.charCodeAt(1) % features.length];
